...
for (let i = 0; i < 10 ; i++){
   ...
   const randomLastNameIndex = Math.floor(Math.random() * cognomi.length);
	...
	const guestName = nomi[randomNameIndex].charAt(0).toUpperCase() + nomi[randomNameIndex].substr(1);
	const guestLastName = cognomi[randomLastNameIndex].charAt(0).toUpperCase() + cognomi[randomLastNameIndex].substr(1);
	let guestFullName = guestName + ' ' + guestLastName;
	guestList.push( guestFullName);
} 
